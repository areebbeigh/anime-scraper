...
from . import printd
from .proxy import BrowserMobProxy
...
def get_proxy():
    return BrowserMobProxy(trust_all_servers=True, capture_content=True)

def get_chrome_webdriver(proxy):
    """
        Returns a chrome webdriver instance with all the addons in /chrome_addons
    """
    options = webdriver.ChromeOptions()
    options.add_argument('--proxy-server={}'.format(proxy.proxy_server.replace('http://', '')))
